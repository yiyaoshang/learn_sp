CREATE PROCEDURE `showself_statistics_sql_data_by_anchor_keep_proc_20160427_bak`(IN p_start_time INT, IN p_end_time INT, IN p_statis_type INT, IN p_deviceType INT,  IN f_cids varchar(255), IN p_cids varchar(255), IN f_type INT, IN p_type INT, IN p_user_name varchar(255))
BEGIN
	...
	IF p_counts > 0 THEN
			...
			DO
				...
				SET @p_sql := CONCAT("INSERT INTO showself_sta_recommend_anchor_tmp
						...
						FROM imeeta_ext_db.shall_admin_recommend_anchor_place_log_",p_part_table," GROUP BY roomid;");
				PREPARE STMT FROM @p_sql;   
				EXECUTE STMT;	
				DEALLOCATE PREPARE STMT; ###释放掉预处理段
